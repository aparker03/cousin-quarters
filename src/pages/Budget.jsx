...
function Budget() {
  ...
  // Load persisted payments or initialize
  useEffect(() => {
    ...
    const init = (arr, label) =>
      Array.from({ length: groupSize }, (_, i) => ({
        name: arr[i]?.name || `Guest ${i + 1}`,
        paid: arr[i]?.paid || false,
      }));

    setHousePayments(init(savedHouse, 'house'));
    setRentalPayments(init(savedRental, 'rental'));
  }, [groupSize]);
  ...
  const handleDownloadCSV = () => {
    ...
    for (let i = 0; i < groupSize; i++) {
      ...
      csv += `${pname},${housePaid},${carPaid},${owes}\n`;
    }
  ...
  return (
    <div className="max-w-6xl mx-auto mt-8 p-4 bg-white rounded-xl shadow-md print:p-0 print:shadow-none print:bg-transparent">
      ...
      <p className="mt-2 text-gray-600">Track who owes what — including rentals, house, and extras.</p>
      ...
      <div className="mt-6 space-y-6">
        ...
        <div className="bg-yellow-50 text-sm text-gray-700 border border-yellow-300 p-4 rounded mt-6">
          ...
          <p className="mb-2">
            Klarna/Affirm will split the house payment monthly. You can pay in full, monthly, or mix.
          </p>
          <p>🚗 Car rentals are paid on pickup day.</p>
        </div>
      </div>
    </div>
  );
